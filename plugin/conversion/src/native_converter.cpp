...
fs::path resolveOriginalStimulusPath(const NativeConverterConfig& config){
    if (!config.originalStimulus.empty())
        return config.originalStimulus;
...
ConversionResult convertNamToClo(const fs::path& inputNam,const fs::path& outputDirectory,
    ...
    const fs::path originalStimulus=resolveOriginalStimulusPath(trainer);
